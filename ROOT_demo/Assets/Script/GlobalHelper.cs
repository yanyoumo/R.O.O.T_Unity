    ...
    public sealed partial class TutorialMgr : MonoBehaviour
    {
        void Awake()
        {
            ...
            _tutorialContent = new[]
            {
                ...
                /*006*/"游戏中另一个重要元素就是"+TmpBracketAndBold("数据")+"；目前游戏中有两种数据："+TMPNormalDataCompo()+"和"+TMPNetworkDataCompo()+"。",
                ...
                /*023*/"成本哪里来？每个单元在棋盘上，就会有一个运营成本，你的总收入只要高于总成本就有收益了。这就是为什么【"+TmpColorRedXml("收入")+"/"+TmpColorXml("损失",Color.green*0.8f)+"】分不同颜色。",
                /*024*/"话说，一般人上班都是一个月拿一次工资。这个收益也是以"+TmpBracketAndBold("周期")+"计算的；这里是游戏最后一个重要元素"+TmpBracketAndBold("周期")+"。",
                ...
                /*029*/"在商店中，同时会有4个"+TmpBracketAndBold("单元")+"可供购买；使用数字键购买，当然，需要按照下面价格支付金钱~",
                /*030*/"但是每个"+TmpBracketAndBold("周期")+"你只能在商店购买一个"+TmpBracketAndBold("单元")+"。",
                /*031*/"商店刷新的方式是这样的：1号位上面的"+TmpBracketAndBold("单元")+"会被移除。后面进行补位，而且越接近1号位，它的价格会越便宜。",
                /*032*/"但是千万不要图便宜就都买下来啊，别忘了"+TmpBracketAndBold("单元")+"的运营费用。来，你自己买几个试试吧。",
                /*033*/"",
                /*034*/"最后，你不会认为你就可以这么安稳的玩下去吧，看这个红色标记。若干个"+TmpBracketAndBold("周期")+"后，下面的单元就会被摧毁。",
                /*035*/"这个黄色状态是提前一"+TmpBracketAndBold("周期")+"的预警。当然，即使是红色标记，如果你及时挪开下面的"+TmpBracketAndBold("单元")+"，就可以回避掉。",
                /*036*/"但是，如果原来不在标记下的"+TmpBracketAndBold("单元")+"，你给挪过去，也是会被摧毁的。嘛，这么做有什么好处就需要你自己考虑考虑了。",
                /*037*/"这种标记在游戏中会随机出现，但是永远都是有预警的。至于怎么利用和回避，就靠你的发挥了。",
                /*038*/"嘛，总之这就是目前全部的系统了。内容还是有点多哈，在游戏中，别忘了按"+TmpBracketAndBold("H")+"随时可以显示提示噢。",
                /*039*/"最后再重复一下游戏结束的条件，如果你没有钱了，或者时间没有了，游戏就结束了。",
                /*040*/"如果刚刚开始玩的话，先争取规定时间内不要破产吧；之后在争取尽可能的多赚钱吧~",
            };
        }
    }
}
